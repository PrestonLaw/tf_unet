...
class SlideDataProvider(BaseDataProvider):
    """
    ...
    def __init__(self, file_names, shape=(512,512), a_min=30, a_max=210):
        
        # Read the slide images using scipy, put them into a list.
        self.images = []
        self.masks = []
        basepath = "/home/preston/Git/tf_unet_backup/train/"

        for file_pair in file_names:
            self.images.append(scipy.misc.imread(basepath + file_pair[0]))
            self.masks.append(scipy.misc.imread(basepath + file_pair[1]))
            
        super(SlideDataProvider, self).__init__(a_min, a_max)
